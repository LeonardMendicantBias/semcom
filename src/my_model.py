...
class Encoder(nn.Module):
    ...
    def forward(self,
        ...
        features = self.vit(frame.flatten(0, 1), None)  # (BT, HW, D)
...
class Decoder(nn.Module):
    ...
    def forward(self, code: torch.LongTensor) -> torch.FloatTensor:
        B, T, _ = code.shape
...
class MaskVideo(nn.Module):
    ...
    def forward(self,
        ...
        code, logits, attn = self.encoder(frames, token_mask)
